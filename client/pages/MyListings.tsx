...
export default function MyListings() {
  ...
  const [displayedAccountIds, setDisplayedAccountIds] = useState<Set<string>>(new Set());
  ...
  const statusStats = useMemo(() => {
    const listed = filteredAccounts.filter((a) => a.transactionStatus === "listed").length;
    const pending = filteredAccounts.filter((a) => a.transactionStatus === "pending").length;
    const sold = filteredAccounts.filter((a) => a.transactionStatus === "sold").length;
    ...
    (
      ...
      setTimeout(() => {
        ...
        if (newAccounts.length > 0) {
          const uniqueNewAccounts = newAccounts.filter((newAccount) => !currentIds.has(newAccount.id));
  ...
  }, [loadMoreAccounts, hasMore, loading, page, displayedAccountIds, filteredAccounts]);
      ...
      <div className="lg:ml-64">
        ...
        <div className="px-4 sm:px-6 py-4 bg-gray-900/30 border-b border-gray-700">
          <div>
            ...
            <p className="text-gray-400 text-sm sm:text-base">Accounts you own and manage</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mt-6">
            ...
            <div className="flex gap-3 sm:gap-4">
              <AdvancedFilter accounts={accounts} onFiltersChange={setFilters} currentFilters={filters} />
            </div>
          </div>
          ...
          {hasActiveFilters(filters) && (
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {getFilterSummary(filters).map((summary, index) => (
                <Badge key={index} variant="outline" className="border-valorant-cyan text-valorant-cyan text-xs">
                  {summary}
                </Badge>
              ))}
              <Button
                ...
                size="sm"
                onClick={() =>
                  setFilters({ gameType: "all", weaponTypes: [], skinTypes: [], specificCombinations: [], searchTerm: "" })
                }
        ...
        <main className="p-4 sm:p-6">
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">{statusStats.listed}</p>
                  </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-valorant-gold">{statusStats.pending}</p>
                  </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">{statusStats.sold}</p>
                  </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-white">
              My Game Accounts
              <span className="text-gray-400 ml-2">({filteredAccounts.length} total, {displayedAccounts.length} loaded)</span>
            </h3>
          </div>
          ...
          {filteredAccounts.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400">You haven't listed any accounts yet</p>
            </div>
          )}
        </main>
      </div>
